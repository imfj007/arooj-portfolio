  ...
  useEffect(() => {
    const loadSound = async () => {
      try {
        ...
        const response = await fetch('/arooj-portfolio/assets/keycap-sounds/press.mp3');
        const arrayBuffer = await response.arrayBuffer();
        const decodedBuffer = await ctx.decodeAudioData(arrayBuffer);
        pressBufferRef.current = decodedBuffer;

        const releaseResponse = await fetch('/arooj-portfolio/assets/keycap-sounds/release.mp3');
